...
- **Scope:**
    -   Next.js 14+ (App Router) execution.
...
- [ ] **Step 4:** Setup Environment Variables.
    - Create `.env.local` template with Supabase and OpenRouter keys.
...
## 4. Verification Plan
- **Automated Tests:**
    - [ ] Run `npm run dev` and verify no crash.
    - [ ] Run `npm run build` to ensure type safety.
- **Manual Verification:**
    - [ ] Visit `localhost:3000` -> Should see Visitor Home.
    - [ ] Visit `localhost:3000/dashboard` -> Should see Admin Placeholder.
